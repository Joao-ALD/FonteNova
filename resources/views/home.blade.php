            ...
            <div class="row align-items-center">
                ...
                <div class="col-lg-5 text-center mt-4 mt-lg-0">
                    <img src="{{ asset('assets/img/icon_brasil.svg') }}" alt="Mapa do Brasil"
                        class="map-illustration img-fluid">
                </div>
                <div id="mapa-interativo-container" style="width: 100%; height: 600px;"></div>
            </div>
        </x-container>
    </section>
...
@push('scripts')
    ...
    <script>
        $(function () {
            // Recebe os dados das iniciativas que o Laravel enviou
            const dadosIniciativas = {!! $dadosIniciativasJson !!};

            // Variável para manter o controle do popover ativo
            let popoverAtual = null;

            // Inicializa o mapa no container
            $('#mapa-interativo-container').vectorMap({
                map: 'br_mill',
                backgroundColor: 'transparent',

                // Define o estilo visual dos estados
                regionStyle: {
                    initial: { fill: '#014BA0' }, // Sua cor azul
                    hover: { fill: '#0066CC', cursor: 'pointer' }
                },

                // A MÁGICA DO POPOVER ACONTECE AQUI
                onRegionClick: function (event, code) {
                    // 1. Destrói qualquer popover que já esteja aberto
                    if (popoverAtual) {
                        popoverAtual.dispose();
                    }

                    const regiaoClicada = event.target; // O elemento SVG do estado clicado
                    ...
                    // 2. Verifica se temos dados para o estado clicado
                    ...
                    // 3. Cria uma nova instância de Popover no estado clicado
                    popoverAtual = new bootstrap.Popover(regiaoClicada, {
                        title: tituloPopover,
                        content: conteudoPopover,
                        html: true, // Permite usar HTML no conteúdo
                        trigger: 'manual', // Nós controlamos quando ele aparece/desaparece
                        placement: 'auto', // Deixa o Bootstrap decidir a melhor posição
                    });
            ...
            //Fecha o popover se o usuário clicar em qualquer lugar fora do mapa
